...
abstract final class Calibration {
  ...
  /// Сколько слов игрок примерно уже знает — то, что показывает экран
  /// результата.
  ///
  /// Считается по накопительным размерам созвездий: ярус B1 означает, что
  /// человек владеет всем, что лежит ниже, а не только словами B1.
  static int estimatedVocabulary(Tier tier, {int constellations = 30}) =>
      ProgressionBalance.starsPerConstellation(tier) * constellations;
}
