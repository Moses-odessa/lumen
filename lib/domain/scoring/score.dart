...
abstract final class ScoreRules {
  ...
  /// Время ответа, приведённое к одному размещению.
  ///
  /// Арена фразы сообщает время от появления задания до **последней** плитки:
  /// на два-девять размещений это неизбежно несколько секунд. Пороги
  /// «быстро» и «легко» рассчитаны на один тап в круге (1.2 с и 2.0 с),
  /// поэтому всякая верно собранная фраза получала `hard` для памяти и
  /// сбрасывала серию «горящего слова» — то есть самая дорогая механика игры
  /// (множитель 2.2–2.5) систематически укорачивала интервал повторения и
  /// повышала трудность того самого слова, которое должна была подтвердить.
  ///
  /// Делится ровно на число слотов, без надбавки на чтение предложения. Это
  /// недооценивает игрока на двух пропусках, где чтение занимает больше, чем
  /// размещение, — но недооценка честнее, чем гарантированный `hard`, и
  /// вводить ещё одно число баланса ради неё рано.
  ///
  /// Одна функция на очки и на память нарочно: «быстро» для очков и «легко»
  /// для памяти — одно и то же наблюдение, и расходиться им нельзя.
  static Duration paceFor(Duration latency, {required int slots}) =>
      slots <= 1 ? latency : latency ~/ slots;
  ...
  ///
  /// Вопрос на слух из-под потолка выведен, и это следствие удаления
  /// `listenTarget`. Правило было «звук стоит по обе стороны деления, и
  /// потолок режет ту половину, где варианты на родном» — то есть платный
  /// вопрос на слух в игре был, просто другой. Его больше нет: варианты на
  /// слух всегда на родном, и под прежним правилом слух перестал бы
  /// оплачиваться выше 40 lm вообще.
  ///
  /// А оставить его без оплаты нельзя не из жалости, а потому что цифры
  /// сходятся в мёртвую точку: [ScoreBalance.speedBonusMinLm] и
  /// [ScoreBalance.recognitionScoreCapLm] равны 40, и они зажимают слух с
  /// двух сторон — ниже 40 нет скоростного множителя, от 40 нет очков. Значит
  /// «переслушивание снимает скоростной множитель» на единственной механике
  /// со звуком не срабатывало бы никогда: снимать нечего.
  ///
  /// Содержательно это тоже верно. Потолок сторожит того, кто знает слово по
  /// написанию и получает очки за повторное его узнавание, — а на слух
  /// написание не показывают. Немецкое слово, выученное глазами, на слух
  /// узнаётся далеко не всегда: `Bahn` и `Bahnhof`, `Sie` и `sie`, `Uhr` и
  /// `Ur`. Это отдельный канал, и яркость звезды о нём не говорит ничего.
