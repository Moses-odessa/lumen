...
class CircleQuestion {
  ...
  /// свою сборку. Пропуск, заполненный верно, но так и не объяснённый, учит
  /// подбирать форму и ничему больше.
  ///
  /// По отправке, а не по заполнению, и правило записано здесь потому, что
  /// оно про задание, а не про виджет. Задание фразы — **порядок**; перевод
  /// сообщает смысл целиком, то есть подтверждает или опровергает выбранную
  /// расстановку. Пока он появлялся при заполнении, это было безобидно —
  /// заполнение и было ответом. С тех пор как ответ отправляет кнопка, тот же
  /// перевод стал подсказкой тому, кто ещё может всё переставить.
