...
class SessionLoader {
  ...
  /// наполняются сами.
  ///
  /// **Пройдя онбординг, игрок приносит сюда 5–6 фраз, и это ровно столько,
  /// сколько нужно знакомству.** Засев даёт в среднем 5.5 фразы (от 3 до 6,
  /// меньше пяти в 7 % забегов — печатает `calibration_diagnostic_test.dart`),
  /// потому что засеять можно только вычитанное, а вычитан один A0, и на нём
  /// калибровка спрашивает шесть кругов ([CalibrationBalance.quotaFor]).
  /// Следствие для второго правила круга записано у сборщика
  /// ([QuestionBuilder]): на полосе из шести пять кандидатов приходятся на
  /// пять мест, ранжировать по совпадению слов не из чего, и сила правила там
  /// равна единице. Ошибкой это не является — знакомство исключением сильнее,
  /// и полоса растёт сама, — но знать об этом надо здесь, потому что размер
  /// полосы задаётся ровно этим методом.
