...
/// производительности из PLAN.md: на B2 небо содержит **весь корпус** —
/// столько звёзд, сколько в нём фраз (сейчас полторы тысячи; было «под три
/// тысячи», когда корпус собирался из словника). Перерисовывать фон вместе с
/// ними на каждом кадре панорамирования нельзя, и требование это от размера
/// корпуса не зависит: он растёт файлами контента, а не кодом.
///
...
class _SkyPainter extends CustomPainter {
  ...
  /// растёт вместе с прогрессом, а не вместе с объёмом контента. Иначе небо
  /// на B2 было бы нарисовано целиком с первого дня: полторы тысячи точек, из
  /// которых игрок трогал три, — и оно перестало бы быть «моим». Счёт шёл на
  /// созвездие («из трёхсот слов»), пока созвездие было темой словника; в
  /// разговорнике их тридцать, и довод держится уже не на одном созвездии, а
  /// на небе целиком.
  void _paintMilkyWay(
    Canvas canvas,
    Size size,
  ...
  void _paintConstellation(
    ...
    // Рисунок созвездия: линия ничего не связывает, см. `SkyLayout._lines`.
    ...
    for (final star in constellation.stars) {
      ...
      // Ореол только у горящих: рисовать блюр на каждой звезде неба —
      // а на B2 их столько, сколько фраз в корпусе, полторы тысячи, — верный
      // способ уронить частоту кадров.
