...
class CalibrationController extends Notifier<CalibrationUiState> {
  ...
  ///
  /// Проверки «а на экране ли мы?» здесь нет, и это не упущение. Тишина вне
  /// экрана — состояние службы ([SpeechService.silence]): пока голос заперт,
  /// сказанное здесь до него не доходит и не копится до возвращения. Своя
  /// проверка в калибровке была бы второй копией правила — а расходятся копии
  /// молча. Сегодня у теста и звучащего центра-то нет
  /// (`CalibrationStep.mode` — всегда `pickTarget`, `promptSpeech` пуст), то
  /// есть онбординг молчал вне экрана по случайности, а не по правилу: добавь
  /// тесту шаг на слух, и жалоба владельца («закрыл окно, а она продолжает
  /// говорить») вернулась бы на первом же экране приложения.
  void _speakPrompt() {
