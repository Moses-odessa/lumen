...
void main() {
  ...
  group('варианты подбираются по совпадению слов', () {
    ...
    test('на полосе из дюжины известных окно не значило ничего', () async {
      // Второй конец, и он важнее первого: **окно молчало ровно там, ради
      // чего правило добавлялось.** Пока пройденных фраз меньше дюжины, окно
      // на дюжину накрывает всю полосу — жеребьёвка пятёрки становится
      // равномерной по всему пройденному, и ранжирование по совпадению слов не
      // влияет ни на что. Это не оценка, а арифметика: одиннадцать кандидатов
      // при окне на двенадцать.
      //
      // Отсюда и устройство проверки: ожидание старого правила здесь — просто
      // средняя похожесть **всей полосы**, и никакого окна воспроизводить не
      // надо. Настоящий сборщик обязан дать заметно больше.
      //
      // Полоса — первая дюжина яруса в авторском порядке, то есть то, что
      // игрок и проходит первым: тема знакомства целиком. Замерено на
      // тридцати шести кругах (дюжина фраз × три механики, по восемь зёрен
      // каждый): 0.155 против 0.073 — **вдвое**. На дюжине, разбросанной по
      // темам, разрыв меньше (1.8×), потому что похожести в такой полосе
      // меньше: правило её не выдумывает, оно перестаёт её выбрасывать.
      final band = [
        for (final row in (await content.phrasesUpTo(Tier.a0)).take(12))
          row.id,
      ];
      final shapes = {
      ...
      for (final id in band) {
        for (final mode in GameMode.values) {
          ...
          // Ожидание окна на дюжину — вся полоса: она короче окна.
      ...
      // Полуторный запас при измеренных 2.1×: правило обязано быть заметно
      ...
      expect(chosen / picks, greaterThan(wholeBand / circles * 1.5),
          reason: 'на полосе из ${band.length} фраз сборщик даёт '
              '${(chosen / picks).toStringAsFixed(4)} против '
              '${(wholeBand / circles).toStringAsFixed(4)} у равномерной '
              'жеребьёвки по всей полосе — то есть у окна на дюжину');
    });
