...
/// под каждой помеченной фразой — английское `casual` или `formal` из
/// внутреннего кода. Ни то, ни другое не зависело от языка, который игрок
/// себе выбрал.
///
/// Числа помеченных фраз здесь больше нет: стояло «432», а счёт пометок
/// живёт в контенте и меняется с каждой пересборкой корпуса, не касаясь ни
/// этого теста, ни `promptTagText`. Что пометки в собранном ассете вообще
/// есть и что все они из набора `promptTags`, проверяется на самом ассете —
/// `test/data/databases_test.dart`.
///
