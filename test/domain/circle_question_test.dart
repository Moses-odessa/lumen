...
void main() {
  ...
  group('несколько слотов', () {
    ...
    test('повторённое слово закрывается своим экземпляром пула', () {
      // «ich» в предложении дважды. Если оба слота указывают на один вариант,
      // второй экземпляр остаётся висеть в пуле, а слот — незакрываемым.
      const built = CircleQuestion(
        itemId: 'doctor_person',
        tier: Tier.a1,
        mode: GameMode.buildPhrase,
      ...
      // Тот же текст, но не тот экземпляр: слоты не взаимозаменяемы.
      expect(built.isCorrectFor(0, 1), isFalse);
      expect(built.isCorrectFor(3, 0), isFalse);
    });
  });
