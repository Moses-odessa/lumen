...
/// Окно **до** множителя — сам расчёт: база плюс объём текста.
///
/// Нужен затем, чтобы проверять расчётные обещания расчётными числами.
/// Множитель ([ScoreBalance.answerWindowFactor]) пришёл с живой игры и может
/// уйти обратно; база, скорость узнавания и потолок — замеры и решения, и
/// сторожить их надо там, где они живут. Считается делением, а не второй
/// копией формулы: копия проверяла бы свою арифметику против чужой.
Duration _counted(int chars) => Duration(
      microseconds: (ScoreBalance.answerWindowFor(chars).inMicroseconds /
              ScoreBalance.answerWindowFactor)
          .round(),
    );
...
void main() {
  ...
  group('ScoreBalance', () {
    ...
    test('окно на ответ длиннее самого медленного порога, который ещё платит',
      ...
      // По ассету шесть самых коротких переводов дают 33 знака.
      //
      // Тридцать три — это корпус, а не правило, поэтому здесь стоит **край
      // самой формулы**: с 28 знаков расчёт обгоняет порог. Круг тесней
      // (меньше пяти знаков на вариант) отменил бы исход «медленный верный
      // ответ» молча — в корпусе такого нет, а если появится, упадёт этот
      // тест, а не темп круга. Что тесный круг корпуса действительно
      // проходит, проверяет `run_screen_test.dart`: там есть сам ассет.
      //
      // Запас с тех пор стал двойным ([ScoreBalance.answerWindowFactor]), и
      // край формулы сдвинулся к нулю: одна удвоенная база — шесть секунд —
      // порог уже обгоняет. Край всё равно проверяется по расчёту, а не по
      // множителю: множитель пришёл с живой игры и может уйти обратно, а
      // обещание «медленный верный ответ существует» должно выжить и без
      // него.
      expect(
      ...
      expect(
        _counted(28),
        greaterThan(ScoreBalance.speedMedium),
        reason: 'обещание держится только множителем',
      );
    ...
    test('окно растёт с объёмом текста и упирается в потолок', () {
      ...
      //
      // База вычитается умноженной: множитель растит и её, иначе «время на
      // текст» тут считалось бы вместе с половиной ориентировки.
      final readA0 = _counted(a0) - ScoreBalance.answerWindowBase;
      final readB2 = _counted(b2) - ScoreBalance.answerWindowBase;
      ...
      // ровно её.
      expect(_counted(0), ScoreBalance.answerWindowBase);

      // Потолок — там, где кончается разговор: ответ, который вспоминают
      // полминуты, не поможет никакой длине фразы. Худший круг, какой можно
      // выдумать (шесть длиннейших фраз вокруг длиннейшего перевода — 792
      // знака), в него упирается, средние ярусы не достают. Настоящие круги
      // не достают тоже: худший из 4500 собранных — 562 знака, см.
      // `ScoreBalance.answerWindowMax`.
      expect(ScoreBalance.answerWindowFor(792),
          ScoreBalance.answerWindowMax * ScoreBalance.answerWindowFactor);
      expect(ScoreBalance.answerWindowFor(562),
    ...
    test('множитель растит и окно, и потолок', () {
      // Решение владельца дословно: «также увеличь расчет таймаута в два
      // раза». Множитель — своё число, а не подмена замеров: база и скорость
      // узнавания остались теми, чем были, и их докстроки не соврали. Здесь
      // сторожится, что он и правда множитель — то есть что окно любого круга
      // ровно во столько раз шире расчёта.
      for (final chars in [0, 28, 130, 423, 562]) {
        expect(ScoreBalance.answerWindowFor(chars).inMicroseconds,
            (_counted(chars).inMicroseconds * ScoreBalance.answerWindowFactor)
                .round(),
            reason: '$chars знаков');
      }

      // И что потолок множится вместе с окном. Оставь его на расчётных
      // пятнадцати секундах — и он срезал бы 1106 кругов из 4500 собранных,
      // из них 624 из 1170 на B2: правило «окно от объёма» на старших ярусах
      // просто исчезло бы, а нужно оно ровно там, где фразы длинные.
      expect(
        ScoreBalance.answerWindowFor(423),
        greaterThan(ScoreBalance.answerWindowMax),
        reason: 'средний круг B2 упёрся бы в неумноженный потолок',
      );
    });
  ...
  group('RevealBalance', () {
    test('пауза после ответа зависит от исхода, а не от механики', () {
      ...
      //
      // Читателей у этих чисел двое, и правило у них одно. У калибровки пауза
      // равна им целиком — озвучки там нет вовсе. У забега они низ паузы,
      // длину которой задаёт озвучка: круг закрывается, когда верный ответ
      // дозвучал, но не раньше этих чисел — иначе на телефоне без голоса
      // (`done` приходит сразу) круг мигал бы мимо глаз. Пока числа лежали
      // внутри `RunController`, у калибровки не было **никакой** паузы: два
      // пути разошлись молча, и удаление констант вернуло бы её туда же.
      final byMode = {
        for (final mode in GameMode.values)
          (
    ...
    test('верх паузы выше любого её низа, но не бесконечен', () {
      // Два числа мерят разное, и путать их нельзя: низ — сколько круг стоит
      // открытым, когда говорить нечем; верх — сколько забег терпит
      // платформу, которая не отвечает. Поэтому сторожится только порядок.
      //
      // Верх ниже низа означал бы паузу, которой нет ни на каком
      // устройстве: круг уходил бы раньше, чем игрок увидел верный вариант.
      expect(RevealBalance.ceiling, greaterThan(RevealBalance.wrong));
      expect(RevealBalance.ceiling, greaterThan(RevealBalance.correct));

      // А слишком щедрый верх — это зависшая игра: игрок, у которого круг
      // стоит открытым десять секунд, читает это не как долгую озвучку.
      // Предложение произносится меньше четырёх секунд (замер в
      // `DeviceSpeechService`), самая длинная фраза корпуса — 118 знаков.
      expect(RevealBalance.ceiling,
