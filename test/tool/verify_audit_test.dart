...
'''),
    ...
    test('поле из словарного слоя — ошибка отчёта', () {
      // У фразы есть текст, вид, регистр и переводы. Части речи, рода и
      // числа ушли вместе со словарным слоем, и находка про них означает,
      // что проверяющему дали устаревший промт.
      final review = reviewReport(
        report('''
findings:
  - id: food_a0_bread
...
'''),
      ...
      expect(review.problems.single, contains('не text, не kind, не register'));
    });

    test('находка про вид фразы принимается, а вид вне набора — нет', () {
      // Забытая пометка `idiom` не находится ничем, кроме вычитки: для
      // валидатора это обычная фраза, и он прав по букве. Поэтому поле `kind`
      // сверка принимает — иначе проверяющему осталось бы написать о такой
      // находке прозой, то есть мимо сверки и мимо правки.
      //
      // Закрытость набора проверяется здесь по той же причине, что и у
      // регистра: `kind` уезжает в базу колонкой NOT NULL, и код вне набора
      // означал бы фразу, про которую нельзя ответить, буквальный у неё
      // перевод или смысловой.
      final review = reviewReport(
        report('''
findings:
  - id: food_a0_bread
    ...
    proposed: idiom
    severity: doubt
    why: "устойчивое выражение, а помечено обычной фразой"
  - id: food_a0_water
    field: kind
    current: phrase
    proposed: sprichwort
...
'''),
      ...
      // Обе находки описывают нынешнее состояние, поэтому обе сошлись:
      // испорчено у второй предложение, а не сверка. Так же устроена и
      // проверка регистра — см. «предложенное значение проверяется».
      expect(review.stale, isEmpty);
      expect(review.places.join('\n'), contains('food_a0_bread/kind'));
      expect(review.problems.join('\n'), contains('вид «sprichwort»'));
      expect(review.clean, isFalse);
    });
