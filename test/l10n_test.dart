...
void main() {
  ...
  test('шаблон непустой и содержит все экраны', () {
    ...
    //
    // Префикс `run` в списке остался, а вот ключа `runNext` («Дальше») в нём
    // больше нет. Он подписывал кнопку под ареной, и охраняла она правило
    // «круг кончается тогда, когда игрок сам решил»: пауза после ответа была
    // числом (420/1100 мс), в которое озвучка не укладывалась, и без кнопки
    // верный ответ обрывался на середине. Правило живо, исполнитель другой —
    // паузу отмеряет теперь сама озвучка (`RunController._holdCircle`), а
    // закрыть её досрочно можно нажатием по арене. Кнопки нет, читателя у
    // ключа не осталось ни одного, и шесть переведённых строк удалены: ключ,
    // который никто не показывает, ищут потом глазами по всем локалям.
    //
