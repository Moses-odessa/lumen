...
/// слов, а вокруг — ровно вынутые слова, без посторонних. Поэтому
/// `phrase_options` уехала целиком: неверных вариантов у этой механики нет.
/// Вместо неё `phrase_orders` — сборки, которые принимаются верными.
///
/// Список сборок нужен потому, что немецкий позволяет вынести в начало почти
/// любой член предложения: «Heute habe ich Zeit» и «Ich habe heute Zeit»
/// правильны оба и означают одно. Собрать предложение из его же слов в другом
/// верном порядке — не ошибка игрока, и говорить ему «неверно» нельзя.
const int contentSchemaVersion = 4;
...
const List<String> contentSchemaDdl = [
  ...
  ''',
  // Сборки, которые принимаются верными. `idx` 0 — та, что задана шаблоном;
  // остальные добавлены вычиткой как равноправные порядки слов.
  //
  // Порядок здесь значим, поэтому он в ключе и его надо запрашивать явно:
  // прошлая таблица фраз держала первичным ключом `(phrase_id, idx, form)`,
  // и база молча отдавала строки по алфавиту — сборщик брал первые пять из
  // семи и систематически выкидывал строчные слова.
  '''
  CREATE TABLE phrase_orders (
    phrase_id TEXT NOT NULL,
    idx INTEGER NOT NULL,
    text TEXT NOT NULL,
    PRIMARY KEY (phrase_id, idx)
