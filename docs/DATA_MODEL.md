...
-- Сборки предложения, которые тоже принимаются верными: немецкий позволяет
-- вынести в начало почти любой член предложения. Заменили `phrase_options`
-- (неверные слова для пропуска) — посторонних слов вокруг фразы больше нет
-- вовсе, вокруг лежат ровно вынутые из неё.
phrase_orders (phrase_id TEXT, idx INTEGER, text TEXT NOT NULL);

-- Перевод фразы целиком на родной язык: проявляется, когда игрок отправил
-- свою сборку («Готово»), а не когда заполнил пропуски. Приходит из файла
-- родного языка, а не из файла фразы.
